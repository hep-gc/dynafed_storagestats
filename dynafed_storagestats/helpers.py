"""
Helpers functions used by the other modules.
"""
...
def convert_size_to_bytes(size):
    """
    Converts given sizes into bytes.
    """
    ############# Creating loggers ################
...
def get_connectionstats(storage_shares, memcached_ip='127.0.0.1', memcached_port='11211'):
    """
...
def process_endpoint_list_results(storage_shares):
    """
    ...
    if len(storage_shares) >= 1:
        for _storage_share in list(range(1, len(storage_shares))):
            _logger.info(
            ...
            storage_shares[_storage_share].stats['filecount'] = storage_shares[0].stats['filecount']
            storage_shares[_storage_share].stats['bytesused'] = storage_shares[0].stats['bytesused']

            # Check if the plugin settings requests the quota from API. If so,
            # copy it, else use the default or manually setup quota.
            if storage_shares[_storage_share].plugin_settings['storagestats.quota'] == 'api':
                storage_shares[_storage_share].stats['quota'] = storage_shares[0].stats['quota']
                storage_shares[_storage_share].stats['bytesfree'] = storage_shares[0].stats['bytesfree']

            else:
                storage_shares[_storage_share].stats['quota'] = storage_shares[_storage_share].plugin_settings['storagestats.quota']
                storage_shares[_storage_share].stats['bytesfree'] = storage_shares[_storage_share].stats['quota'] - storage_shares[_storage_share].stats['bytesused']

            # Might need to append any issues with the configuration settings
            storage_shares[_storage_share].status = storage_shares[0].status
            storage_shares[_storage_share].debug = storage_shares[0].debug
